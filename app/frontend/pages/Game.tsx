...
export default function Game(props: GameProps) {
	...
	const data = useMemo(
		...
		[props.mode, props.difficulty, props.cards]
	);
	...
	useEffect(() => {
		...
		if (!cards || cards.length === 0) {
			setError(
				mode === 'specific' ? '特定札が選択されていません' : 'カードデータの読み込みに失敗しました'
			);
			setIsLoading(false);
			return;
		}
...
${gameMode === 'specific' ? '特定札練習' : gameMode === 'practice' ? '練習' : 'ランダム'} ${gameMode === 'random' ? difficultyLabel : ''}で${score.total.toLocaleString()}点獲得！
